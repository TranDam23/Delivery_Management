...
export function OrderDetailPage({ orderId }: { orderId: string }): React.JSX.Element {
  ...
  return (
    <>
      ...
      <Card className="border-sky-400/25 bg-sky-400/5"><div className="flex items-center gap-3"><MapPin className="text-sky-300" size={18} /><div><CardLabel>Kho được phân tự động</CardLabel><p className="mt-1 text-[11px] text-dt-muted">Kho con được chọn theo xã/phường, quận/huyện và tỉnh/thành phố của địa chỉ.</p></div></div><div className="mt-4 grid gap-3 text-[11px] md:grid-cols-2"><div><p className="text-dt-muted">Kho lấy hàng</p><p className="mt-1 font-medium text-sky-200">{pickupWarehouse ? `${pickupWarehouse.code} · ${pickupWarehouse.name}` : "Chưa xác định"}</p><p className="mt-1 text-dt-muted">{pickupWarehouse ? [pickupWarehouse.ward, pickupWarehouse.district, pickupWarehouse.province].filter(Boolean).join(", ") : "Điều phối viên sẽ kiểm tra khi lập tuyến."}</p></div><div><p className="text-dt-muted">Kho giao cuối</p><p className="mt-1 font-medium text-sky-200">{deliveryWarehouse ? `${deliveryWarehouse.code} · ${deliveryWarehouse.name}` : "Chưa xác định"}</p><p className="mt-1 text-dt-muted">{deliveryWarehouse ? [deliveryWarehouse.ward, deliveryWarehouse.district, deliveryWarehouse.province].filter(Boolean).join(", ") : "Điều phối viên sẽ kiểm tra khi lập tuyến."}</p></div></div></Card>

...
export function AddressFormPage({ contactId, addressId }: { contactId: string; addressId?: string }): React.JSX.Element {
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit} className="max-w-[900px]"><Card className="gap-4"><CardLabel>Thông tin địa chỉ</CardLabel><div className="grid gap-4 md:grid-cols-2"><TextField label="Tên người nhận" required value={form.recipient_name} onChange={(event) => update("recipient_name", event.target.value)} /><TextField label="Số điện thoại" required inputMode="tel" value={form.phone} onChange={(event) => update("phone", event.target.value)} /></div><TextField label="Địa chỉ cụ thể" required placeholder="Số nhà, tên đường" value={form.address_line} onChange={(event) => update("address_line", event.target.value)} /><div className="grid gap-4 md:grid-cols-3"><TextField label="Phường/Xã" value={form.ward} onChange={(event) => update("ward", event.target.value)} /><TextField label="Quận/Huyện" value={form.district} onChange={(event) => update("district", event.target.value)} /><TextField label="Tỉnh/Thành phố" value={form.province} onChange={(event) => update("province", event.target.value)} /></div><label className="flex w-fit cursor-pointer items-center gap-2 text-[12px] text-dt-muted"><input type="checkbox" checked={form.is_default} onChange={(event) => update("is_default", event.target.checked)} className="h-3.5 w-3.5 accent-[var(--dt-yellow)]" /> Đặt làm địa chỉ mặc định</label>{editing && form.is_default ? <p className="text-[11px] text-dt-muted">Liên hệ luôn cần một địa chỉ mặc định; hệ thống sẽ giữ địa chỉ này nếu bạn không chọn địa chỉ khác.</p> : null}{error ? <p role="alert" className="rounded-md border border-dt-red/40 bg-dt-red/10 px-3 py-2 text-[12px] text-red-200">{error}</p> : null}<div className="flex gap-2 pt-2"><Link href={`/contacts/${contactId}/addresses`} className={buttonClassName("secondary")}>Hủy</Link><Button type="submit" disabled={submitting}>{submitting ? "Đang lưu..." : editing ? "Lưu thay đổi" : "Lưu địa chỉ"}</Button></div></Card></form>
    </>
  );
}

...
export class UserRepository {
  ...
  async findPublicByEmail(email: string): Promise<PublicUser | null> {
    const { data, error } = await this.database
      ...
      .from("users")
      .select(
        "id, role_id, full_name, email, phone, avatar, province, warehouse_id, status, created_at, updated_at, last_login_at",
      )
      .eq("email", email)
      .maybeSingle();
  ...
  async listUsers(params: ListUsersParams): Promise<ListUsersResult> {
    ...
    let query = this.database
      ...
      .from("users")
      .select(
        "id, role_id, full_name, email, phone, avatar, province, warehouse_id, status, created_at, updated_at, last_login_at, roles(code, name), warehouses(id, code, name, province, warehouse_level)",
        { count: "exact" },
      )
      .range(from, to)
    ...
    return {
      users: (data ?? []) as unknown as ListUsersResult["users"],
      total: count ?? 0,
    };
  }
  ...
  async createUser(input: {
    ...
    const { data, error } = await this.database
      ...
      .from("users")
      .insert(input)
      .select("id, role_id, full_name, email, phone, avatar, province, warehouse_id, status, created_at, updated_at, last_login_at")
      .single();
